...
pub fn run_combo_backtest(klines: &[Kline], settings: ComboSettings, filter: &IndicatorFilter) -> BacktestResult {
    ...
    for i in 1..klines.len() {
        ...
        // Entry Signal: Golden Cross + indicator confirmation
        ...
        // DCA Logic: Price drops
        if position_size > dec!(0) && k.close < average_price * (dec!(1) - settings.dca_percent) {
            if balance >= settings.buy_amount {
                let units = settings.buy_amount / k.close;
        ...
        // Take Profit Logic
        if position_size > dec!(0) && k.high > average_price * (dec!(1) + settings.tp_percent) {
            let sell_price = average_price * (dec!(1) + settings.tp_percent);
